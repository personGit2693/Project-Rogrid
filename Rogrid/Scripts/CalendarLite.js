...
/*_Current Day*/
var getCurrentDay_Rogrid = new Date().getDate();
var dayValue_Rogrid = getCurrentDay_Rogrid;
...
/*_Current Month*/
var getCurrentMonth_Rogrid = new Date().getMonth();
var monthValue_Rogrid = getCurrentMonth_Rogrid+1;
...
/*_Current Year*/
var getCurrentYear_Rogrid = new Date().getFullYear();
/*_Current Year*/


/*_Calendar Lite Hidden input for Calendar Lite Value*/
var hiddenCalLiteVal_ID_Rogrid = document.getElementById("calLite_ID");
//hiddenCalLiteVal_ID_Rogrid.value = getCurrentYear_Rogrid+"-"+monthValue_Rogrid+"-"+dayValue_Rogrid;
...
var availableSchedFound = []; /*Editable*/
/*Months Text Collection*/
...
const renderCalLite = (getNumberOfDays_Param, newMonth_Param, newYear_Param) => {
	/*_Print Month and Year Inner Text*/
	const calLiteMonthText_Rogrid = document.getElementsByClassName("calLiteMonthText_RoClass")[0];
	const monthTxt_Rogrid = monthsTxt_Array[newMonth_Param];
	...
	calLiteYearText_Rogrid.innerText = newYear_Param;
	/*_Print Month and Year Inner Text*/
	...
	const firstDay_Rogrid = monthTxt_Rogrid+" "+1+", "+newYear_Param;
	...
	/*_Creating days per week loop*/ /*Editable*/
	for(let index=1; index <= getNumberOfDays_Param; index++){
		...
		dayTxtDiv_Rogrid.addEventListener("click", function(){
			getCalendarLiteValue(newYear_Param, newMonth_Param+1, this);
		});

		dayTxtDiv_Rogrid.addEventListener("click", printTimebox); /*From PrintTimebox_Request.js*/


		/*_ _Setting Attribute per day number div*/
		var attriMonth_Rogrid = newMonth_Param+1;
		...
		dayTxtDiv_Rogrid.setAttribute("data-dayDate", newYear_Param+"-"+attriMonth_Rogrid+"-"+attriDay_Rogrid);
		/*_ _End Setting Attribute per day number div*/		


		/*_ _Editable*/
		const createdDatePerLoop_Rogrid = newYear_Param+"-"+attriMonth_Rogrid+"-"+attriDay_Rogrid;
		if(availableSchedFound.includes(createdDatePerLoop_Rogrid)){
			dayTxtDiv_Rogrid.style.backgroundColor = "green";
			dayTxtDiv_Rogrid.style.color = "#ffffff";
		}else if(!availableSchedFound.includes(createdDatePerLoop_Rogrid)){
		...
		const createdDate_Rogrid = monthTxt_Rogrid+" "+index+", "+newYear_Param;		
...
const changeCalLiteMonth = (addMinusMonth_Param) => {
	if(addMinusMonth_Param === "minus"){
		if(getCurrentMonth_Rogrid !== 0){
			getCurrentMonth_Rogrid -= 1;
			renderCalLite(getNumberOfDays(getCurrentMonth_Rogrid, getCurrentYear_Rogrid), getCurrentMonth_Rogrid, getCurrentYear_Rogrid);
		}else if(getCurrentMonth_Rogrid === 0){
			getCurrentMonth_Rogrid = 11;
			getCurrentYear_Rogrid -= 1;
			renderCalLite(getNumberOfDays(getCurrentMonth_Rogrid, getCurrentYear_Rogrid), getCurrentMonth_Rogrid, getCurrentYear_Rogrid);
		}
	}else if(addMinusMonth_Param === "add"){
		if(getCurrentMonth_Rogrid !== 11){
			getCurrentMonth_Rogrid += 1;
			renderCalLite(getNumberOfDays(getCurrentMonth_Rogrid, getCurrentYear_Rogrid), getCurrentMonth_Rogrid, getCurrentYear_Rogrid);
		}else if(getCurrentMonth_Rogrid === 11){
			getCurrentMonth_Rogrid = 0;
			getCurrentYear_Rogrid += 1;
			renderCalLite(getNumberOfDays(getCurrentMonth_Rogrid, getCurrentYear_Rogrid), getCurrentMonth_Rogrid, getCurrentYear_Rogrid);
		}
	}
...
const getCalendarLiteValue = (calLiteYearVal_Param, calLiteMonth_Param, dayNumDivElem_Param) => {	
	...
	for(let index=0; index<dayNumber_Rogrid_Coll.length; index++){
		if(dayNumber_Rogrid_Coll[index] == dayNumDivElem_Param){
			dayNumber_Rogrid_Coll[index].style.backgroundColor = "#44D3A3"; /*_ _Editable*/
			dayNumber_Rogrid_Coll[index].style.color = "#ffffff"; /*_ _Editable*/
		}else if(dayNumber_Rogrid_Coll[index] != dayNumDivElem_Param){
			/*_ _Editable*/
			if(availableSchedFound.includes(dayNumber_Rogrid_Coll[index].getAttribute("data-dayDate"))){
